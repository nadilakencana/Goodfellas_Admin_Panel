...
class POSController extends Controller
{
    ...
    public function PartMenuDiscount(){
        if(Sentinel::check()){
            ...
            //  $itemSubCategory = Menu::where('id_sub_kategori', $subCategory->id)->get();
             ...
             return view('POS.part_lain.menu_discount', compact('itemMenu',
             'Category',
             'subCategory',
             //'itemSubCategory'
            ));
        ...
        if(Sentinel::check()){
           $itemMenu = Menu::where('id_kategori',$id)->where('custom', false)->get();
            ...
            //  $itemSubCategory = Menu::where('id_sub_kategori', $subCategory->id)->get();
             ...
             return view('POS.part_lain.menu_kat', compact('itemMenu',
             'Category',
             'subCategory',
             //'itemSubCategory'
            ));
    ...
    public function PartAllMenu(){
        if(Sentinel::check()){
             ...
             $subCategory = SubKategori::all();
            //  $itemSubCategory = Menu::where('id_sub_kategori', $subCategory->id)->get();
             return view('POS.part_lain.all_menu', compact('itemMenu',
             'Category',
             'subCategory',
             //'itemSubCategory'
            ));
    ...
    public function PartSubMenu($id){
        if(Sentinel::check()){
            ...
            //  $itemSubCategory = Menu::where('id_sub_kategori', $subCategory->id)->get();
             ...
             return view('POS.part_lain.sub_menu', compact('itemMenu',
             'Category',
             'subCategory',
             //'itemSubCategory'
            ));
        ...
        if (Sentinel::check()) {
            ...
            try{
                ...
                if(!empty($request->get('target_detail'))){
                    ...
                    if(!empty( $detail->last_print) && $detail->update == 0){
                            $detail->update = 1 ;
                            $detail->last_print = null ;
                    }
                    if(!empty( $detail->last_print) && $detail->update == 1){
                    ...
                    if(empty($order->id_type_payment)){
                        ...
                        if ($detail->save()) {
                            ...
                            if ($request->has('discount')) {
                                foreach ($request->discount as $discounts) {
                                    ...
                                    $Discount->total_discount = $detail->total * $rateDis;
